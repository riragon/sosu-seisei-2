...
Sosu-Seisei-2 は、素数への理解を深めつつ、使うたびに「新しい疑問」と「新しい解析のアイデア」を引き出すことを目指した、素数探究のための実験ツールです。

本アプリは **2つのモード**で構成されています:

- **Sosu-Seisei**: 素数の生成 / 可視化（Generator / π(x) / Gap / Density / Spiral）
- **Sosu-Analyze**: 生成済みの素数バイナリ（`.bin`）を入力として解析・検証（LastDigit / Mod30 / PRHS / Validation）

Generator で「巨大範囲の素数」を手に入れ、
π(x) で「全体の傾向」を掴み、
Gap / Density / Spiral で「局所構造」や「模様」を眺める。

さらに Sosu-Analyze で、末尾桁・合同類・PRHS などの“癖”を集計し、素数ファイルの検証も行えます。

こうした一連の流れをワンクリックで試せるのが本アプリの特徴です。ぜひ、ご自身の PC で素数の世界を遊び倒してみてください。

## 想定している利用シーン

- 高校～大学レベルの数学授業で「素数の分布」を体感的に理解したいとき
- 自由研究や卒論で素数の密度・ギャップ・π(x) の性質を調べたいとき
- 数値実験・HPC ベンチマークとして大規模素数生成を行いたいとき
- ブログや動画で「素数の世界」を視覚的に紹介したいとき
...
- **素数バイナリ（`.bin`）の解析・検証**（末尾桁 / mod 30 遷移 / PRHS / Validation）

---

## スクリーンショット（Sosu-Seisei）
...
> Sosu-Analyze のスクリーンショットは今後追加予定です。

---

## 画面モード紹介（2モード）

起動後、画面左上のタイトルをクリックすることで **Sosu-Seisei / Sosu-Analyze** を切り替えられます（計算中は切り替え不可）。

### Sosu-Seisei（5タブ）— 生成・可視化
...
### Sosu-Analyze（5タブ）— `.bin` 入力の解析・検証

Generator などで作った **素数バイナリ（例: `primes.bin`）** を読み込み、集計・解析・検証を行います。

#### 1) LastDigit — 末尾桁の出現率

- 末尾 1/3/7/9 の出現率などをテーブル化して観察します。

#### 2) Mod30 Trans — mod 30 遷移

- mod 30 の合同類における遷移パターンを集計して観察します。

#### 3) mod 30 PRHS — mod 30 の PRHS 分析

- mod 30 の枠組みで PRHS（規則性）を集計して観察します。

#### 4) mod 210 PRHS — mod 210 の PRHS 分析

- より細かい合同類（mod 210）で PRHS を集計して観察します。

#### 5) Validation — 素数ファイルの検証

- 入力 `.bin` の整合性（単調増加、合成数混入の検出など）をチェックします。
