...
use crate::seisei::output::PrimeWriter;
use crate::seisei::sieve_math::{integer_sqrt, simple_sieve};
...
pub fn generate_primes_cpu(
    ...
    use crate::seisei::memory;
    let num_threads = rayon::current_num_threads();
    let optimal_segment_size =
        memory::calculate_optimal_segment_size(cfg.memory_usage_percent, num_threads, wheel_type);
    ...
    log::info!("ホイールタイプ: {wheel_type:?}, セグメントサイズ: {segment_size}");
    ...
    log::info!(
        "Processing ~{total_segments} segments in groups of {group_size} (total {total_groups} groups)"
    );
